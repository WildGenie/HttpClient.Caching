namespace HttpClient.Caching
{
    ...
    public class CachingHandler : DelegatingHandler
    {
        ...
        private static readonly HttpStatusCode[] CacheableStatuses =
        {
        ...
        private readonly Func<HttpRequestMessage, bool> _ignoreRequestRules;
            ...
            IVaryHeaderStore varyHeaderStore = null)
        {
            ...
            VaryHeaderStore = varyHeaderStore ?? new InMemoryVaryHeaderStore();
            DefaultVaryHeaders = new[] {"Accept"};
            ResponseValidator = response =>
            {
                ...
                if(!response.IsSuccessStatusCode || (response.Headers.CacheControl == null) ||
                   response.Headers.CacheControl.NoStore)
                    //  || response.Headers.CacheControl.NoCache was removed. See issue
                    return ResponseValidationResult.NotCacheable;
                ...
                response.Headers.Date = response.Headers.Date ?? DateTimeOffset.UtcNow;
                    // this also helps in cache creation
                var dateTimeOffset = response.Headers.Date;

                /*TraceWriter.WriteLine(
                    string.Format("CachedResponse date was => {0} - compared to UTC.Now => {1}", dateTimeOffset,
                ...
                if((response.Content.Headers.Expires != null) &&
                   (response.Content.Headers.Expires < DateTimeOffset.UtcNow))
                    return response.Headers.CacheControl.ShouldRevalidate(MustRevalidateByDefault)
                        ? ResponseValidationResult.MustRevalidate
                ...
                if((response.Headers.CacheControl.SharedMaxAge != null) &&
                   (DateTimeOffset.UtcNow >
                    response.Headers.Date.Value.Add(response.Headers.CacheControl.SharedMaxAge.Value)))
                    return response.Headers.CacheControl.ShouldRevalidate(MustRevalidateByDefault)
                        ? ResponseValidationResult.MustRevalidate
                        : ResponseValidationResult.Stale;
            ...
            _ignoreRequestRules = request =>
            {
                ...
                return request.Headers.CacheControl != null && request.Headers.CacheControl.NoStore;
            };
...
/*        public string[] StarVaryHeaders { get; set; } // TODO: populate and use*/
        ...
        /// <summary>
        ///     Allowing clients to customise how to handle caching errors. Mainly designed for Cache Store temporary errors or
        ///     persistent erros.
        ///     Throw in the action if you do not want to ignore. You can use for logging purposes.
        ///     A common usage is to use set it to IgnoreExceptionPolicy which basically would ignore exceptions as if the
        ///     CachingHandler is not used
        ...
        private bool? IsFreshOrStaleAcceptable(HttpResponseMessage cachedResponse, HttpRequestMessage request)
        {
            ...
            if(cachedResponse.Headers.CacheControl.MaxAge.HasValue) // Note: this is MaxAge for response
                staleness =
                    DateTimeOffset.Now.Subtract(responseDate.Value.Add(cachedResponse.Headers.CacheControl.MaxAge.Value));
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                cachedResponse = await _cacheStore.TryGetValue(cacheKey);
                cachingHeader.DidNotExist = cachedResponse == null;
                ...
                if(!cachingHeader.DidNotExist.Value) // so if it EXISTS in cache
                {
                ...
                {
                    DoPutValidation(request, cachingHeader, cachedResponse);
                    return await base.SendAsync(request, cancellationToken);
                }
                ...
                if(validationResultForCachedResponse == ResponseValidationResult.OK)
                {
                    ...
                    return cachedResponse.AddCachingHeader(cachingHeader);
                }
                ...
                {
                    DoCacheValidationForGet(request, cachingHeader, cachedResponse);
                }
            }
            catch(Exception ex)
            {
               ...
               (serverResponse.StatusCode == HttpStatusCode.NotModified))
            {
                ...
                return cachedResponse.AddCachingHeader(cachingHeader); // EXIT !! _______________
            }
            ...
            switch(validationResult)
            {
                ...
                case ResponseValidationResult.OK:
                    ...
                    // store the cache
                    await _cacheStore.AddOrUpdate(cacheKey, serverResponse);
                ...
                default:
                    /* TraceWriter.WriteLine("{0} - ResponseValidationResult. Other",
                     ...
                     TraceWriter.WriteLine("{0} - Before TryRemove", TraceLevel.Verbose,
                         request.RequestUri.ToString());*/
                    await _cacheStore.TryRemove(cacheKey);
                    /* TraceWriter.WriteLine("{0} - After AddOrUpdate", TraceLevel.Verbose,
                         request.RequestUri.ToString());*/

                    cachingHeader.NotCacheable = true;
            ...
            return serverResponse.AddCachingHeader(cachingHeader);
        }
